...
//  QMessageBox::information(nullptr, QObject::tr("Help"), cmdLineParser.getHelpText());
    QMessageBox *msg = new QMessageBox(QMessageBox::Information, QObject::tr("Help"),
                       ...
                       QMessageBox::Ok, this);
    msg->setInformativeText(tr("More info can be found at ParsiCoin.Net in Help section"));
...
void MainWindow::walletOpened(bool _error, const QString& _error_text) {
  if (!_error) {
	...
	setWindowTitle(QString(tr("%1 - ParsiCoin Wallet %2")).arg(Settings::instance().getWalletFile()).arg(Settings::instance().getVersion()));
...
void MainWindow::walletClosed() {
  ...
  setWindowTitle(QString(tr("ParsiCoin Wallet %2")).arg(Settings::instance().getVersion()));
...
void MainWindow::createTrayIcon()
{
    ...
    QString toolTip = QString(tr("ParsiCoin Wallet %1")).arg(Settings::instance().getVersion());
