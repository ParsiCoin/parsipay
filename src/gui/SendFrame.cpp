...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("parsicoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "parsicoin:");
    }
    if(!_request.startsWith("parsicoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with parsicoin:"), QtCriticalMsg));
      return;
    }
